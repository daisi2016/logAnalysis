...
public class SimpleUtil {
	 ...
	 */
	/*
	 * public static String getProxyIp() { HttpServletRequest request = JsfContext.getRequest(); String ip = ""; // System.out.println(request.getServerName());
	 * // System.out.println(request.getLocalName()); // System.out.println(request.getHeader("host")); //
	 * System.out.println(JsfContext.getRequest().getRequestURL()); String host = request.getHeader("host"); if (host.contains(":")) { host =
	 * host.split(":")[0]; } if (!isBlank(host)) { ip = host; } else { StringBuffer http = request.getRequestURL(); String url = ""; if (http != null) { url =
	 * http.toString(); if (url.contains("http://")) { String url$ = url.split("//")[1]; ip = url$.split("/")[0]; } } } return ip; }
	 */
